...
description: On supported Codex Desktop hosts, research an organization, create or update its reusable organization pack, and produce brand-specific WeChat Official Account articles with user-selected micro-component count, optional SVG interaction modules, selectable style routes and optional generated backgrounds, plus structured content, visual QA, static-safe WeChat transport, and draft handoff. Use for any organization’s recruitment, event, project, educational, partnership, recap, or announcement article. Do not use from another LLM/harness or for unrelated social posts or generic research with no WeChat deliverable.
---
...
This executable release supports **Codex Desktop only** on the locked platform row. Its schemas may inform a future port, but another LLM, CLI agent, cloud harness, Linux, Windows, or Intel Mac is not an executable target until a reviewed adapter, login routes, full forward test, and release lock are shipped. ChatGPT is a Codex-controlled planning/review and image-source dependency, not a replacement execution host.

At the first turn for every new clone or workspace, state this support boundary and the phase-specific prerequisites before reading organization material. Read [references/host-prerequisites.md](references/host-prerequisites.md) first. Confirm that the operator has opened the exact clone in Codex Desktop, installed the same-release `org-wechat-studio`, `chatgpt-web-image-route`, and `ardot-wechat-publisher` packages, installed and bound `codex-with-chatgpt` to this exact workspace, and reloaded the Skill registry. For `migration/authoring/full`, prepare the single built-in-Browser ChatGPT session and request login only at a real wall. For `bootstrap/authoring/delivery/full`, connect Ardot Remote, complete Ardot OAuth/web login as needed, and prove exact file/root access. For `delivery/full`, separately prepare the exact WeChat account session or execution-time API route. A clone, Skill file, saved profile, or old Cookie proves none of these live conditions.

When the source checkout is available, run its read-only `release_skills.py clone-check --phase <phase>` declaration first. It may prove local files and binaries, but it intentionally cannot set `live_session_ready` or authorize reading source material. Stop outside Codex Desktop or while any required local/live item is unresolved.

## Mandatory startup creative intake

After declaring host prerequisites and before reading source material, ask one grouped question that captures all four article-level production choices:

1. micro-component count from `0` to `4`;
2. whether to author SVG interaction candidates (`true` means 2–3 semantic modules; `false` means a zero-module `static-selected` article);
3. the style route (an existing approved organization route, source-zero calibration for a new route, or an explicitly named reviewed style grammar); and
4. whether to generate raster background images (`true` means one calibrated family; `false` means Ardot-native surfaces, gradients and open vectors only).

Do not silently choose defaults. Record the confirmed answer under `article.production_preferences` with `status: confirmed`, `confirmed_by: user|editor`, `micro_component_count`, `use_svg`, `style_route`, and `generate_backgrounds`; keep `article.route` equal to `style_route`. After the storyboard, choose exactly that many semantically useful roles in `visual_kit.selected_roles`. A substantive change to any of these four values requires one grouped reconfirmation before generation or layout.
...
Before opening source material, creating an organization pack, generating an article asset, or touching Ardot/WeChat, read [references/runtime-preflight.md](references/runtime-preflight.md) and bind the current Codex Desktop session to the project skill hashes and semantic capabilities. At the beginning of every workflow/organization migration into a new clone, machine, Codex session, adapter/provider route, or changed trusted bundle, run `migration` first. It binds the runtime and provider session without generating or inspecting a synthetic RGBA calibration image; RGBA quality is evaluated only on real transparent article assets and never blocks source reading. An attempted move to another LLM/harness stops as unsupported; migration is not an on-the-fly compatibility certification. Then use `full` when an exact current Ardot file/root already exists. For a new organization with no Ardot workspace, use `bootstrap`, verify `ardot.create`, create only the blank design/page, then immediately rerun the requested terminal phase (`full` by default, or `authoring` when explicitly scoped) with its canonical file/root. `bootstrap` does not require a WeChat target or login. Use `delivery` only for an existing reviewed Ardot article.
...
2. Do not handwrite a runtime profile, host export, or `loaded/available` status. The release manifest must be the create-once copy installed under the Skill root by `release_skills.py`; a repository commit or working-tree file is not proof that the active harness loaded it. Current Codex Desktop has no authoritative `host.registry.export` callable, so initialize a non-attested, same-session census from the verified release, reviewed adapter allowlist, and the identifiers actually visible in the model's current registry:
   ...
   This path must report `current-session-model-visible-intent`, `host_attested_registry: false`, and later live probes. `build-census` remains a contract-development entrypoint for a future reviewed adapter, but it is not a supported end-user route in this release and must not be used to claim another harness is compatible. The Codex census maps actual registry identifiers to image, Browser, Ardot, WeChat, and optional signer/lease capabilities. Formal current-session acquisition is operationally accepted after the current runtime/provider session, canonical request, create-once ingestion, exact raw bytes, and the real asset's selected quality chain pass; it remains operator/harness-trusted, `host_attested=false`, and `portable=false`.
...
4. Execute the binding report's ordered setup actions before reading organization material, except the explicitly optional `run-migration-rgba-route-probe`. For Codex Desktop, load the wrapper and C2C Skills, close tunnel/setup/project/connector/workspace-identity checks separately, claim one visible built-in-Browser ChatGPT tab, and ask once for login/CAPTCHA/2FA/consent only when the real page blocks. Computer Use and external browsers are forbidden for ChatGPT. Ardot MCP OAuth and Ardot web login are separate probes. For WeChat API mode, resolve the provider without opening a token-bearing editor URL; only a selected UI route opens the credential-free platform entry. `bootstrap` prepares only `ardot.create`; `delivery` does not prepare ChatGPT or generation.

5. Do not run a synthetic RGBA startup probe. Create an `org-wechat-runtime-session-evidence-v1` record that binds the report's exact nonce/digest and installed release/registry/adapter/route values plus the current provider session ID. Finalize this lightweight binding before registering the first formal generated micro asset, not before reading source material. The legacy `run-migration-rgba-route-probe` action is non-blocking diagnostics only.

6. Finalize the current-session runtime binding. On a normal Codex host, consume the exact runtime-session evidence and require `operational_ready: true` while preserving `phase_ready: false`:
   ...
   The old image-probe-based portable migration finalizer remains an optional audit route and is never required for source reading or normal authoring:
...
5. Before the first full article for an organization or route, read [references/visual-calibration.md](references/visual-calibration.md) and create two or three small Ardot calibration strips. A reviewed style preset may be offered as one route-level option only when explicitly selected; it never replaces source-zero as the default or skips current-organization calibration:
   ...
   Pass the confirmed background choice: use `generated-family` when raster backgrounds were selected and `native-surfaces` otherwise. Show only a hero, chapter, real-photo treatment, micro-visual language sample, and density strip for each direction. The micro-visual calibration sample is made from editable Ardot vectors/fills only; it is not a generated asset or RGBA probe. The first real selected article component after the storyboard performs the image-route test. If generated backgrounds are used, create one master plus 1–3 companions in the same family during calibration. Preserve each approved unmarked master, create a distinct provenance-watermarked final derivative before asset registration, and require authenticated local detection; never watermark a real photograph, logo, QR code, transparent micro component, SVG, or QA screenshot. Declare one family-wide `surface_mode`, a normalized copy-safe rectangle, the body text hex color, minimum contrast `4.5`, and maximum copy-zone luminance deviation `0.10`; register final opaque PNGs and rerun `orgs.py validate` so the actual composited pixels—not prompt prose—prove continuity and readability. If generated backgrounds were not selected, omit `background_family` and calibrate one continuous native Ardot reading surface without raster atmosphere assets. For `expressive-native`, approve at least two named construction recipes; each recipe needs at least two non-font techniques and at least two editable text/accent layers. A font change alone is never an expressive treatment. Stop until these checks, one route, and its Ardot file/page/node are recorded under `organization.visual.calibration`. A provisional organization never proceeds to a full article.
6. For an article, read [references/article-schema.md](references/article-schema.md), [references/storyboard.md](references/storyboard.md), [references/interaction-composition.md](references/interaction-composition.md), [references/expressive-typography.md](references/expressive-typography.md), [references/ardot-workflow.md](references/ardot-workflow.md), and [references/organic-layout.md](references/organic-layout.md). Write and approve a 4–10 chapter narrative storyboard before generating visuals:
   ...
   Use two bounded editorial checkpoints, not per-asset prompts. A new
   organization/route gets one visual-calibration approval. Then bind the
   article storyboard, interaction plan, and exactly the confirmed number of selected visual-kit slot briefs
   into one digest and request one grouped article approval. Within that
   unchanged digest, generate the selected slots continuously and do not ask again
   for each component. A processor-proven native-Alpha failure automatically
   advances to the already-bound controlled-key fallback without another
   confirmation. Reconfirm only after a substantive subject/style/scope change
   or an actual login, CAPTCHA, 2FA, or consent wall. Draft creation remains the
   default terminal state; formal publication and group send each require their
   own fresh, exact confirmation.

7. Add an approved `interaction_plan` that matches the startup choice. With `use_svg: true`, `dynamic-default` contains 2–3 semantic interaction modules distributed across distinct chapters: two use `early` + `middle`; three use `early` + `middle` + `late`. Count one continuous reader task as one module even when it contains several transport instances: four department reveal cards are one module and four instances. Each instance binds current block copy to a unique fallback key and `sha256:<64 hex>`. With `use_svg: false`, use `static-selected`, `target_module_count: 0`, and an empty module list; the confirmed preference is the one required user decision. Missing target-account runtime certification never cancels authored SVG candidates; it selects the static equivalent at delivery.
8. Build the selected article-specific micro-illustration kit (count `0` uses `status: not-requested`, empty selected roles and assets):
   ...
   On Codex Desktop, use `chatgpt-web-image-route` by default for every selected role from `floating-spot`, `section-transition`, `inline-explainer`, and `closing-motif`. Ask ChatGPT for an article-grounded provider-original PNG, download that original, and retain it under `assets/generated/`. Every formal article source uses `org-wechat-provider-image-acquisition-v2`: it binds the verified installed-release census, the adapter-declared `generation_route_id`, the same-session runtime binding, canonical request metadata, and a create-once Browser ingestion report whose target is the exact raw processor source. Browser/provider recovery events are separate from source attempts: resume and reread the same request first and never duplicate an unknown request. The raw source need not already be acceptable RGBA: use `scripts/prepare_micro_cutout.py --require-native-alpha` for a suitable true-alpha original, or the slot's exact controlled key and `--key-color` route when that is the safer source. These are per-real-asset choices, not a startup gate. Never use a screenshot, preview Canvas, clipboard image, or copied remote URL.

   Current-session article acquisition can run through the normal API or standalone CLI after the exact same-session runtime binding and every acquisition→ingestion→raw→derivative check validate. Record `current-session-operator-harness-trusted`, `host_attested=false`, and `portable=false`; do not describe that result as an independent or portable proof. `live_provider_acquisition_authority(callback)` is a compatibility-named optional trusted-harness policy hook only: `True` leaves assurance unchanged, while `False` or an exception blocks. Ordinary Python cannot make this hook unforgeable. The checked-in Codex Desktop adapter therefore needs no hook or signer for normal current-session authoring. Read [references/provider-acquisition-authority.md](references/provider-acquisition-authority.md) for the complete boundary.

   Create a distinct file under `assets/derived/`, then run `python3 -I -S "$ORG_WECHAT_RUNTIME_ROOT/scripts/secure_runner.py" "$ORG_WECHAT_RUNTIME_ROOT/scripts/inspect_asset.py" DERIVED --role ROLE`. A provider claim of transparency is untrusted; only the downloaded pixels and complete derivation chain decide current-session operational use, while protected Ed25519 receipts separately upgrade portable assurance. Bind every report slot exactly as `kit.<role>`. Across all selected roles, the accepted provider-original SHA, provider request ID, acquisition binding, and final derivative SHA must each be distinct; recropping or copying one raw image can never satisfy multiple roles. Create exactly the confirmed number of native Ardot components from the derivatives, then record each component `file_url`, `node_id`, exact `name`, asset ID, and derivative SHA in `article.visual_kit.assets`. Rerun the plan against the current chain (or with the protected portable trust store) and do not continue while `ready_for_layout` is false. The relaxed raw-source admission never permits RGB, opaque pseudo-RGBA, checkerboard, white/colored backplates, halo, debris, or matte in a registered derivative.
...
9. After every selected native micro component exists—or immediately when the confirmed count is zero—build the deterministic Ardot assembly manifest:
...
- Every article uses the confirmed `micro_component_count` from 0–4 and exactly that many distinct article-specific generated derivatives and selected semantic roles. Reusing identity files, old decorations, raw source downloads, or documentary photos never satisfies a selected slot; count zero creates no visual kit assets.
- Reject final generated derivatives that are rectangular, framed, generic, text-bearing, or only appear transparent because a checkerboard was baked into the pixels. A raw RGB or controlled-key source is acceptable input to the cutout processor, but an RGB final, all-opaque RGBA conversion, or ChatGPT transparency claim is never sufficient. Bind the original source, prompt, generation route, processor/config, derivation report, derivative SHA, and final Alpha inspection before registration.
...
- Before creating assets for a new account or article type, generate a route-specific asset plan. It distinguishes reusable assets, missing real photographs, allowed generated illustrations, and identity files that must come from the user:
  ...
  Pass the confirmed route and background choice explicitly; use
  `native-surfaces` instead of `generated-family` when raster backgrounds were
  declined. The generic plan does not create micro-component placeholders.
...
- When `use_svg: true`, create 2–3 semantic interaction modules. A module is one reader task and one static-equivalent region; its child SVGs or swipe triggers are transport instances, not extra modules. When false, use zero modules. Do not count decorative motion, selected micro illustrations, or expressive typography toward this budget.
...
- Do not begin the article root until every selected micro-visual role exists as a native Ardot component. Use them beside text, across transitions, along a continuous path, and near the ending—not as rectangular panel backgrounds. A confirmed zero count skips this gate without reusing old assets.
...
- A micro component is a partial-width editorial accent, not a miniature poster or a full-row raster panel. Keep each raster/illustration layer at or below 72% of the 390 px row and the whole component at or below 82%. For 2 or more selected components, use both left and right offsets and visible scale variation. Require `min(3, count)` distinct offsets, composition relations, and screenshot sections.
...
- When `generate_backgrounds: true`, register one AI background master plus 1–3 same-family companions with a shared family ID and explicit variants before full layout. The whole family uses one light/dark surface mode; its normalized copy-safe zone must be near-solid and maintain at least 4.5:1 contrast with the declared body text color. Run `orgs.py validate` after the final PNGs are registered. When false, omit the family and use continuous native Ardot surfaces only. Neither mode may mix black/white chapter surfaces or let copy merge with its reading surface.
...
- missing, stale, unsafe, self-reported-only, or failing runtime preflight for the active phase; a binding-only report presented as phase readiness; a different project or provider-wrapper Skill SHA; C2C doctor/workspace output, a local derivation report alone, or model-authored download receipt presented as host-route proof; an article acquisition whose adapter/census/runtime-session/route/ingestion/raw chain is not exact; any current-session claim of host attestation/portability, a configured policy hook that denies or fails, an invalid/tampered portable receipt or repository-writable trust store; a generic executor presented as ImageGen/Browser/Computer Use; an untrusted Ardot/WeChat URL; or a target file/account mismatch;
...
- missing/unconfirmed `article.production_preferences`; a component count outside 0–4; a selected style route that differs from `article.route`; an incomplete `article.visual_kit`; selected roles/assets whose count differs from `micro_component_count`; selected derivatives that reuse raw-source SHA / provider-request ID / acquisition-authority binding values; or any selected role with a non-`kit.<role>` lineage slot, absent original-download evidence, missing source/prompt/route/processor/config/report lineage, RGB or all-opaque final pixels, non-RGBA8/undecodable Alpha, low-Alpha residue, chromatic/neutral halo, detached debris, oversized transparent canvas, clipped subject, rectangular/rounded/solid/textured matte, stale cutout SHA/evidence, or missing native Ardot component node evidence;
...
- when `generate_backgrounds: true`, a missing generated family, master, 1–3 companions, normalized copy-safe zone, one declared surface mode, 4.5:1 body-text contrast, near-solid copy surface, pixel-checked tonal continuity, or mismatched family metadata; when false, any generated raster atmosphere background in the article or a native reading surface with a black/white jump or inadequate contrast;
...
- missing `interaction_plan`; a plan that conflicts with the confirmed `use_svg` choice; a selected dynamic article outside the 2–3 semantic-module budget; child instances, decoration, micro illustrations, or display type used to pad the count; repeated chapter/placement bands; ungrounded instance copy; duplicate fallback keys or semantic hashes; or a later static exception without a specific user/editor-confirmed reason;
...
- missing measured evidence for any selected micro-component role; an image wider than 72% of the row; a component wider than 82%; fewer than `min(3, count)` screenshot sections, distinct offsets, or composition relations; for count 2–4, missing both left/right offsets or visible scale variation; framed copy; or copy-bearing micro components without native text nodes, 22 px / 1.35× primary scale contrast, and a second non-frame emphasis technique;
- missing `information_density` / `background_surface_unity` / `reading_surface_contrast` / `expressive_typography` / `art_type_construction` / `no_baked_art_text` screenshot checks; when generated backgrounds are selected, missing `background_family_coherence`; unhashed/non-390 px Ardot exports; fewer than five density samples; measured body-text contrast below 4.5; `compact-editorial` major gaps outside 24–40 px; body line-height outside the selected mode; or an accidental empty region larger than 20% of a sampled section;
