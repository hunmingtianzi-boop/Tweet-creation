...
- Run `python3 -I -S scripts/secure_runner.py scripts/validate_transport_fidelity.py HANDOFF_JSON --intended-html OUTPUT/wechat.html --live-root-export LIVE_ROOT_JSON --live-root-receipt LIVE_RECEIPT_JSON --require-live-root` and stop on any error. The receipt must be issued and Ed25519-signed by the harness from the actual Ardot tool response; the repository has no signing key and must never fabricate it.
- Produce WeChat-safe inline HTML only with `python3 -I -S scripts/secure_runner.py scripts/compile_wechat.py --transport-fidelity HANDOFF_JSON --live-root-export LIVE_ROOT_JSON --live-root-receipt LIVE_RECEIPT_JSON --output OUTPUT --check`. Never call the article-JSON renderer for delivery; `--authoring-preview` is explicitly non-delivery and does not create `wechat.html`.
...
- Require `compile-report.json.artifact_binding.wechat_html` to bind the exact HTML path-identity hash, device/inode, SHA-256, byte length, handoff SHA and transport revision, and require `artifact_binding.live_root_receipt` to bind the original signed receipt. Immediately before upload/copy/paste, rerun `validate_transport_fidelity.py HANDOFF_JSON --html OUTPUT/wechat.html --live-root-export LIVE_ROOT_JSON --live-root-receipt LIVE_RECEIPT_JSON --compile-report OUTPUT/compile-report.json --require-compile-report`; copying the directory, rewriting the report or changing/substituting the file requires a newly signed intended-path receipt and recompilation.
...
- Export `wechat-saved-draft-readback-v1` with target account, draft ID, observation time and one entry per frozen chapter: section mapping, normalized visible-text-node IDs/hash, all hosted asset IDs plus downloaded files whose SHA-256 is recomputed, and a hash-bound `390 × chapter_height` reopened-draft screenshot. The host must issue `wechat-host-saved-draft-receipt-v1`, Ed25519-signed from the actual API/browser observation and bound to the account/draft, live receipt, HTML, compile report and complete readback bytes. Run `validate_transport_fidelity.py HANDOFF_JSON --html OUTPUT/wechat.html --live-root-export LIVE_ROOT_JSON --live-root-receipt LIVE_RECEIPT_JSON --compile-report OUTPUT/compile-report.json --require-compile-report --readback READBACK_JSON --readback-receipt READBACK_RECEIPT_JSON --require-readback`; local files and plausible WeChat URLs without the host receipt do not prove readback.
...
- Copy only from the validated frozen-layer `wechat.html`, not from an article-JSON authoring preview, previous clipboard, screenshot splice, or old experiment output.
