...
- Feed the host adapter's normalized current-root export, or a resolved raw current-root capture with `--bindings SEMANTIC_BINDINGS.json`, into the deterministic exporter. See the same-release runtime's `references/audit-repair-integration.md` for the bounded raw adapter and required evidence. Freeze and validate `production_intent` against the actual exported layers; changed user choices require reconfirmation. The output directory must be new; the exporter refuses overwrite, symlink, non-regular, collision, stale capture, and non-terminal attribution inputs:
...
- When the reviewed handoff contains requested non-static interactions, compilation emits dynamic and information-equivalent static payloads plus one selected payload. A `static-selected` handoff contains zero non-static interaction layers and delivery selects its static transport; a compatibility artifact with a dynamic filename must not be represented as authored interaction. Missing, stale, mismatched, or untrusted mobile evidence also selects static. A dynamic selection requires an exact candidate/readback hash, target account/draft, fresh real-PNG iOS and Android preview captures with distinct device sessions, and either a portable host signature a fresh in-process `CurrentSessionMobileAuthority` response from an isolated trusted embedding harness, or the explicit nonportable editor-review draft route below. Serialized flags, session IDs, Booleans, and host-trace files cannot unlock dynamic output.
- Bootstrap dynamic evidence only after the static candidate has created the mapped draft. Compile `--session-draft --interaction-probe` from a newly captured live root into a separate output, then `save-draft` updates that existing media ID with the dynamic payload. This scope is `current-session-interaction-probe`: it may be reopened for sanitizer/mobile evidence, but it may never create a draft or pass publication preflight. After real iOS/Android evidence exists, either (a) have an isolated trusted embedding harness call the compiler programmatically with the exact mobile profile, readback and an in-process `current_session_mobile_authority`, or (b) use a `portable-signed` mobile profile verified against the protected host trust root. The standalone CLI exposes no live-authority flags. It also supports explicit real editor review: use `current-session-editor-reviewed`, null signature fields and the exact-draft/both-client editor_review record described in runtime `references/audit-repair-integration.md`; pass `--accept-editor-mobile-review` to compile, save-draft and capture-readback. This accepts actual human evidence for this draft only, never independently attests the host and never authorizes publication. Without that explicit acceptance, unsigned profiles select static. Update and reopen the same mapped draft; if any gate fails, restore the static payload there instead of creating a duplicate.
...
- Collect actual 320/390/430px saved-article-container screenshots and measured native text bounds/font/overflow using the runtime's read-only `measure_wechat_viewport.js` (never put JavaScript in the article). Pass the bound review via `capture-readback --viewport-review FILE`; without it final readback cannot pass. Container-relative typography is a candidate until the real sanitizer and render pass. Independently fresh screenshots may have identical pixels; compare local and global errors separately from provenance.
...
python3 -I -S "$ORG_WECHAT_RUNTIME_ROOT/scripts/secure_runner.py" \
  ...
  --viewport-review "$EXTERNAL_READBACK_ROOT/viewport-review.json"
```
