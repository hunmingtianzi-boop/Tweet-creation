...
当前执行入口见 [审计修复集成说明](audit-repair-integration.md)：原始节点 + 语义绑定可经 `export_ardot_handoff.py --bindings` 归一化；handoff 同时冻结生产选择与 transport。文字字号/字距改用章节容器相对单位，必须在真实保存草稿完成 320/390/430px `viewport_review` 才能通过终态回读。全图和局部像素误差分别验收；相同像素本身不判伪造，来源真实性单独核验。静态/动态候选、编辑者真机人审、宿主回调与签名审计的证据等级不能互换。
