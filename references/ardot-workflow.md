...
12. 为 Hero、章节、证据、复杂区块和 CTA 建立独立截图验收 JSON，记录 390 px 下的字号、行高、字距、段距、内容占用率与最大空洞比例，运行 `build_visual_review.py`。五类节点、密度样本与十二项检查全部通过才投递。
...
- 所有正文按 390 px 检查；默认 `compact-editorial` 为 15–17 px、1.45–1.62 行高、-0.2–0 px 中文字距和 8–14 px 段距。正文不能松散漂浮，也不能挤成文字墙。
- Hero 与章节标题可使用组织专属的艺术字处理，但必须由 Ardot 原生文字构成并保持可编辑。允许宋黑对比、1–4 px 套色错版、刻意断行、低透明度超大章节数字与克制的尺度变化；最上层主标题必须保持高对比，正文不得跟随艺术化，禁止把 AI 生成中文或栅格化标题塞进图片。
