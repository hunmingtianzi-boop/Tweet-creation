...
| 创作选项 | 还没问用户就回落到旧的四组件、动态或生成底图默认 | `production_preferences` 必须为 `confirmed`，记录 user/editor、`micro_component_count: 0..4`、`use_svg`、与实际 route 一致的 `style_route` 和 `generate_backgrounds` | 在读取材料后、建立生成/交互/底图计划前一次确认；修改选项后重建下游计划 | 不得用历史默认、编辑器能力或目标账号传输降级反向改写用户选择 |
...
| RGBA 源 | 把 raw 必须原生 Alpha 与最终真透明门禁混为一层 | 每个 slot 在请求前声明 `native-alpha` 或 `controlled-key`，Browser 原图 create-once 摄取固定源 SHA/bytes；终态 derivative 统一重算 RGBA8、robust Alpha、紧裁切、open-edge 和 matte/halo/debris | native 直接规范化，受控纯色按已声明 key 本地分离；某一 raw 路线质量失败可换源重生 | 不使用截图、预览 canvas、剪贴板图或远程 URL；raw 路线选择不得放宽 final derivative |
| 生图超时 | provider 仍在生成或 Browser 断线，却被当成像素失败、重复发请求 | 分开 `provider-pending` / `completed-await-download` / `provider-terminal-failed` / `browser-control-unavailable`，恢复事件不计 source attempt | 先只读恢复同一 C2C session/request；明确 terminal failure 才可以同 mode/同 prompt 新 request ID 重提 | 未知状态禁止重复提交；登录/超时不得冒充任一源图模式的像素质量结论 |
| 正式生图采集 | 操作者手写 provider ledger/route/host trace，或把 Python callback 冒充宿主签名 | acquisition v2 绑定 verified release census、adapter route、当前会话 runtime binding、每次 canonical request、create-once ingestion/raw SHA+bytes 和终态 RGBA 像素链；prepare/register/pack/ready 全部重验，第一枚文章相关小组件直接承担路由质量实测 | current-session 可 operational accept，但固定 operator/harness-trusted、`host_attested=false`、`portable=false`；可选 callback 只能 veto；portable 由受保护 Ed25519 双签名升级 | 旧 v1、伪 route、raw/像素篡改、callback 否决仍阻断；`lambda True` 绝不得产生鉴真或可携带宣称 |
| 终态抠图 | raw 路线合法，但 derivative 边缘仍有白边/色 halo/烟雾/碎片或底板 | RGBA8、非零 Alpha、紧裁切、open-edge、matte/halo/debris 与三底像素验收 | 重生成或重做安全分离；不能降低门槛修补 | raw 是 native-alpha 或 controlled-key 都不等于可进 Ardot |
| 已选微组件 | 数量/角色与已确认偏好不符、资产重复、带底板或变成一张通栏大图 | `micro_component_count: 0..4` 与 `selected_roles` 数量一致；`N > 0` 时 distinct derived SHA 及各自 Ardot node/asset 证据，`N = 0` 时集合为空 | 拒绝进入全文装配，先重做缺失的已选组件；零组件则按原生构图继续 | 装饰、艺术字或交互子实例不得凑数 |
| 微组件排版 | 小组件占满横版，有字就打框，文字不突出 | 390 px 截图重算宽度、偏移、尺度、关系、native text node 与强调技法；已选数量 `N` 至少覆盖 `min(3, N)` 个截图区段/偏移/关系，`N >= 2` 还需左右错落和尺度变化 | 保留主体透明、部分宽度、左右错落和非框强调；`N = 0` 时证据为空/`not-applicable` | 任一实际组件超 82%、图超 72% 或含框文字均阻断 |
| 底图/原生画布 | 同文混用黑/白/大色块，纹理、主色或光向断裂 | `generate_backgrounds: true` 时验证家族 master/companion 的空间频率、纹理、光向、surface mode 及色调连续性；`false` 时验证同 route Ardot 原生 surfaces/渐变/可编辑矢量层的连续性 | 回到小样阶段重做所选底层模式；正文字独立 | 纯色 CSS 块不得伪装成 Ardot 底图导出，也不得用伪生成图凑家族 |
...
| 动态组件 | 未按已确认的 SVG 选择创作，或 JS 被清洗、SVG 状态丢失 | `use_svg: true` 必须是 `dynamic-default`、2–3 个 semantic modules，使用无 ID 自触发 SVG/SMIL 或 CSS 横滑，每实例绑定 fallback key/语义 SHA 和 Ardot 三态证据；`false` 必须是 `static-selected`、0 modules | 动态模式编译同一 revision 的 dynamic/static 双 payload，目标端证据不足时只在投递层选 static；静态模式不生成交互候选 | 结构回读通过不等于手机端能运行；投递降级不得改写创作选项 |
